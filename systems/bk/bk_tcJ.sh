...
# =================================================================================
# Targeted Journal Entry Corruption
# =================================================================================
log_message "${YELLOW}Phase 3: Targeted Journal Entry Corruption${RESET}"
...
log_message "Targeting journal file: $CURRENT_TXN_FILE"

# Find actual entry positions by looking for the known content
ENTRY_POSITIONS=$(find_entry_positions "$CURRENT_TXN_FILE")

if [ -z "$ENTRY_POSITIONS" ]; then
    log_message "${RED}No entries found in the journal file${RESET}"
    exit 1
fi
...
for position in $ENTRY_POSITIONS; do
    corrupt_specific_entry "$CURRENT_TXN_FILE" "$position"
    count=$((count + 1))
    if [ $count -eq 3 ]; then
        break
    fi
...
log_message "${GREEN}Journal entry corruption complete${RESET}"
