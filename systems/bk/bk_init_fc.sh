...
# Find the current ledger file (it will be in current/N.log)
CURRENT_LEDGER_FILE="$LEDGER_DIR/current/0.log"

if [ ! -f "$CURRENT_LEDGER_FILE" ]; then
    log_message "${RED}No ledger file found at expected location${RESET}"
    exit 1
fi

log_message "Corrupting ledger file: $CURRENT_LEDGER_FILE"

# Corrupt the entire file by overwriting it with random data
dd if=/dev/urandom of="$CURRENT_LEDGER_FILE" bs=1M count=1 conv=notrunc
...
log_message "Attempting to read ledger $LEDGER_ID"
java -jar $CLIENT_JAR read $LEDGER_ID > $BK_LOG 2>&1

# =================================================================================
# Log Analysis
# =================================================================================
log_message "${YELLOW}Phase 6: Error Analysis${RESET}"
echo -e "\n${YELLOW}Key log entries:${RESET}"
grep -E "Recovery|checksum|replicat|exception" $BK_LOG | tail -n 20
